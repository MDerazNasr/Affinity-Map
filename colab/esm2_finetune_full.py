"""
colab/esm2_finetune_full.py

Full-spec ESM-2 LoRA episodic fine-tuning — for GPU (Google Colab T4/A100).

Settings match the original experimental spec:
  EPOCHS=30, EP_TRAIN=200, EP_VAL=100, EVAL_EP=1000, ESM_MAX_LEN=512

Run from project root:
    python colab/esm2_finetune_full.py
...
# Episodic training — full spec
N        = 5
K_TRAIN  = 5
Q        = 10
EPOCHS   = 30
EP_TRAIN = 200       # episodes per training epoch
EP_VAL   = 100       # episodes per val epoch
LR       = 1e-4
GRAD_CLIP = 1.0
SEED     = 42
CKPT_EVERY = 5       # intermediate checkpoint every N epochs
...
# Evaluation
K_SWEEP  = [1, 2, 5, 10, 20]
EVAL_EP  = 1000      # matched episodes per K value

ESM_MAX_LEN = 512    # full-length; T4/A100 handles this with gradient checkpointing
BATCH_SIZE  = 16
AA_SET = set("ACDEFGHIKLMNPQRSTVWY")

# Device: CUDA required (this script is for GPU only)
if not torch.cuda.is_available():
    raise RuntimeError(
        "No CUDA GPU found. This script requires a GPU.\n"
...
def embed_sequences(model, tokenizer, seqs, batch_size=BATCH_SIZE):
    """Embed raw sequences → (N, 320) L2-normalised tensor. Grad-safe."""
...
def build_lora_model():
    ...
    model.gradient_checkpointing_enable()   # saves ~6x peak VRAM at ~2x compute cost
    model.print_trainable_parameters()
    return model.to(device), tokenizer


def build_frozen_model():
    """Frozen ESM-2 for matched comparison (no LoRA, no grad)."""
    ...
    with open(CURVE_PATH, "w", newline="") as cf:
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for ep_idx in range(EP_TRAIN):
                ...
                if (ep_idx + 1) % 50 == 0:
                    print(f"  ep {ep_idx+1}/{EP_TRAIN}  "
                          f"loss={np.mean(tr_losses):.4f}", flush=True)
            ...
            print(f"Epoch {epoch:3d}/{EPOCHS}  "
                  f"loss={tl:.4f}  train={ta:.3f}  val={va:.3f}", flush=True)
            ...
            if epoch % CKPT_EVERY == 0:
                ckpt = f"{CKPT_DIR}_ep{epoch}"
                model.save_pretrained(ckpt)
                print(f"  --> intermediate checkpoint: {ckpt}", flush=True)
...
def run_matched_eval(lora_model, tokenizer, frozen_model, test_fams, K):
    ...
    for ep_idx in range(EVAL_EP):
        ...
        # k-mer ProtoNet (matched same sequences)
        ...
        protos_km = protos_km / (np.linalg.norm(protos_km, axis=1, keepdims=True) + 1e-8)
        ks_q_n    = ks_q / (np.linalg.norm(ks_q, axis=1, keepdims=True) + 1e-8)
        preds_km  = (ks_q_n.astype(np.float64) @ protos_km.astype(np.float64).T).argmax(1)
        kmer_accs.append((preds_km == qy_np).mean())

        if (ep_idx + 1) % 200 == 0:
            print(f"  K={K}: {ep_idx+1}/{EVAL_EP} episodes...", flush=True)
...
def main():
    ...
    print(f"  {len(all_fams)} families → "
          f"train={len(train_fams)} val={len(val_fams)} test={len(test_fams)}")

    lora_model, tokenizer = build_lora_model()
    print(f"\nTraining for {EPOCHS} epochs "
          f"({EP_TRAIN} train ep + {EP_VAL} val ep per epoch)...")
    best_val = train(lora_model, tokenizer, train_fams, val_fams)
    print(f"\nBest val_acc = {best_val:.3f}")

    # Reload best checkpoint for eval
    print("\nLoading best checkpoint for evaluation...")
    base_eval = EsmModel.from_pretrained(HF_MODEL).to(device)
    lora_eval = PeftModel.from_pretrained(base_eval, CKPT_DIR).to(device)
    lora_eval.eval()

    frozen_model = build_frozen_model()

    print(f"\nRunning matched evaluation "
          f"({EVAL_EP} episodes × {len(K_SWEEP)} K-values)...")
    ...
    for K, r in results.items():
        print(f"ESM-2 LoRA & "
              ...
              f"{r['lora_vs_frozen']['diff']*100:+.1f} pp ({r['lora_vs_frozen']['sig']}) & "
              f"{r['lora_vs_kmer']['diff']*100:+.1f} pp ({r['lora_vs_kmer']['sig']}) \\\\  % K={K}")
